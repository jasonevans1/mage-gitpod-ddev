...
if [ "${MAGENTO_EDITION}" = "enterprise" ]; then
    ddev magento config:store:set btob/website_configuration/company_active "1"
    ddev magento config:store:set btob/website_configuration/sharedcatalog_active "1"
    ddev magento config:store:set btob/website_configuration/negotiablequote_active "1"
    ddev magento config:store:set btob/website_configuration/quickorder_active "1"
    ddev magento config:store:set btob/website_configuration/requisition_list_active "1"
    ddev magento config:store:set btob/default_b2b_payment_methods/available_payment_methods "braintree_ach_direct_debit,braintree_applepay,banktransfer,cashondelivery,checkmo,braintree,payflow_advanced,payflow_link,payflowpro,braintree_googlepay,braintree_local_payment,free,braintree_paypal,paypal_billing_agreement,payflow_express_bml,paypal_express_bml,paypal_express,payflow_express,hosted_pro,companycredit,purchaseorder,braintree_paypal_vault,braintree_cc_vault,payflowpro_cc_vault,braintree_venmo"
    ddev magento config:store:set btob/default_b2b_shipping_methods/applicable_shipping_methods "0"
    ddev magento config:store:set btob/website_configuration/purchaseorder_enabled "1"
    ddev magento config:store:set catalog/magento_catalogpermissions/enabled "1"
    ddev magento config:store:set catalog/magento_catalogpermissions/grant_catalog_category_view "1"
    ddev magento config:store:set catalog/magento_catalogpermissions/grant_catalog_product_price "1"
    ddev magento config:store:set catalog/magento_catalogpermissions/grant_checkout_items "1"
    ddev magento config:store:set sales/product_sku/my_account_enable "1"
fi &&

if [ "${INSTALL_HYVA}" = "YES" ] && [ ! -z "${HYVA_COMPOSER_TOKEN}" ] && [ ! -z "${HYVA_COMPOSER_PROJECT}" ]; then
    # TODO: find a more reliable way to check hyva/default theme ID
    if ddev magento dev:theme:list | grep -q "spectrum"; then
        ddev magento config:store:set design/theme/theme_id 6
    else
        ddev magento config:store:set design/theme/theme_id 5
    fi

    ddev magento config:store:set customer/captcha/enable 0
fi
